...
class OncopSeeder extends Seeder
{
    public function run(): void
    {
        // Make sure codigo_cuenta is UNIQUE so we don't duplicate on multiple seed runs
        $this->pdo->exec("ALTER TABLE cuenta_contable DROP CONSTRAINT IF EXISTS cuenta_contable_codigo_cuenta_key;");
        $this->pdo->exec("ALTER TABLE cuenta_contable ADD CONSTRAINT cuenta_contable_codigo_cuenta_key UNIQUE (codigo_cuenta);");
        ...
        $stmt = $this->pdo->prepare("INSERT INTO cuenta_contable (codigo_cuenta, denominacion_cuenta, tipo_cuenta) VALUES (?, ?, ?) ON CONFLICT (codigo_cuenta) DO NOTHING");
