...
class OnapreSeeder extends Seeder
{
    public function run(): void
    {
        // Asegurar que codigo_plan_unico sea UNIQUE para evitar duplicados en reinicios
        $this->pdo->exec("ALTER TABLE plan_unico_cuentas DROP CONSTRAINT IF EXISTS plan_unico_cuentas_codigo_key;");
        $this->pdo->exec("ALTER TABLE plan_unico_cuentas ADD CONSTRAINT plan_unico_cuentas_codigo_key UNIQUE (codigo_plan_unico);");
...
('3.04', 'Transferencias y Donaciones')
...
SQL);
        ...
        $stmt = $this->pdo->prepare("INSERT INTO plan_unico_cuentas (codigo_plan_unico, denominacion) VALUES (?, ?) ON CONFLICT (codigo_plan_unico) DO NOTHING");
